...
def compute_ap(recall, precision):
    ...
    method = 'continuous'  # methods: 'continuous', 'interp'$$$$$$$#################原先是interp
    if method == 'interp':
...
def kmeans_targets(path='../coco/trainvalno5k.txt', n=9, img_size=960):  # from utils.utils import *; kmeans_targets()
...
def plot_images_ygx(imgs, targets, paths=None, fname='images.jpg'):
    # Plots training images overlaid with targets
    imgs = imgs.cpu().numpy()
    ...
    for i in range(1):
        ...
        boxes = xywh2xyxy(targets[targets[:, 0] == i, 2:6])
        print(boxes)
        # boxes[[0, 2]] *= w
        # boxes[[1, 3]] *= h
        # print(boxes.shape)
        ...
        for i in range(boxes.shape[0]):
            # print(boxes[0][i])
            c1, c2 = (int(boxes[i][0]*2048), int(boxes[i][1]*2048)), (int(boxes[i][2]*2048), int(boxes[i][3]*2048))
            print(c1)
            cv2.rectangle(imgs, c1, c2, (0,255,0), thickness=2)
...
if __name__ == "__main__":
    ...
    kmeans_targets (path = '/home/zj/detection/yolov3-new/data/TTKtrain.txt')